...
- LLM 抽取边界通过 provider 配置、环境变量和 JSON/Pydantic 校验接入，测试使用 fake transport，不依赖真实 API key。
...
- LLM 抽取器已提供 deterministic demo、DeepSeek 和 Qwen OpenAI-compatible provider；默认路径不调用外部 API，真实 provider 需要配置 `DEEPSEEK_API_KEY` 或 `DASHSCOPE_API_KEY`。
...
2. 为 DeepSeek/Qwen provider 增加真实网页评测样本和失败统计，比较规则抽取、deterministic demo 和真实 provider 的收益。
