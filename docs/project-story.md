...
- 可视化输出：生成 Markdown 报告、本地岗位 HTML Dashboard 和评测摘要 HTML Dashboard。
...
> 构建 Web 自动任务 Agent，完成 AI 实习岗位搜索、指定 JD 打开、JD 抽取、匹配分析和可视化报告；设计可测试的浏览器执行边界、20 任务评测集、公开招聘页 fixture、seed URL 评测和真实浏览器 smoke 评测，输出任务成功率、有效岗位数、平均访问页面数和失败原因分布等指标。
...
- 116 个自动化测试覆盖模型、浏览器边界、抽取、验证、匹配、技能缺口汇总、简历输入、seed URL、JSON 导出、评测 JSON 导出、运行历史、CLI 版本入口、环境自检、存储、报告、Dashboard 交互、workflow、LangGraph 编排、图导出、公开招聘页 fixture 和失败分类。
