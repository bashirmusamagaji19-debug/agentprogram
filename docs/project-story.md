...
这是一个面向 AI 工程 / AI 应用实习的 Web 自动任务 Agent。系统可以自动发现 AI 实习岗位，抽取结构化 JD，验证岗位有效性，结合用户技能和简历文本生成匹配分析，并输出 Markdown 报告、本地 HTML Dashboard、输入轨迹和 20 任务评测报告。
...
> 基于 Python 构建 Web 自动任务 Agent，实现 AI 实习岗位发现、指定 JD 打开、结构化 JD 抽取、岗位有效性验证、简历匹配分析、SQLite 持久化和报告生成。设计 `BrowserClient` 抽象隔离 `browser-use` session adapter 与 deterministic fake browser，并将工作流拆分为 Planner、Browser、Extractor、Verifier、Matcher、Reporter、Dashboard 和 Evaluation 模块；提供 LangGraph 节点编排路径。构建 20 任务评测集，统计任务成功率、有效岗位数和平均访问页面数；当前 demo 评测中 20/20 任务完成，任务成功率 1.00。
...
- 125 个自动化测试覆盖模型、浏览器边界、抽取、验证、匹配、技能缺口汇总、简历输入、seed URL、JSON 导出、评测 JSON 导出、运行历史、CLI 版本入口、环境自检、fixture URL 列表、存储、报告、Dashboard 交互与输入轨迹、workflow、LangGraph 编排、图导出、公开招聘页 fixture、失败分类和 URL 级错误明细。
