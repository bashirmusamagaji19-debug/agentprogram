...
- 真实 `browser-use` session adapter 已接入，`HttpPageLoader` 可用于抓取真实 URL 的 title/body 文本；评测报告已能统计 `browser_error`、`no_pages`、`no_extracted_jobs`、`verification_filtered` 等失败类别。
- LLM 抽取器已提供 deterministic demo、DeepSeek 和 Qwen provider；真实 provider 需要配置 `DEEPSEEK_API_KEY` 或 `DASHSCOPE_API_KEY`；DeepSeek provider 已通过全链路 smoke 验证。
- 匹配模块已升级为两层架构：规则优先（≤0 cost）+ LLM 语义 fallback（`--llm-match` / `--llm-match-provider`）；DeepSeek 语义匹配已验证可用。
- Dashboard 是静态 HTML，已支持筛选、排序、输入轨迹和 Agent 执行轨迹展示。
- 真实站点评测集目前有 4 个 URL（OpenAI ×2 + Anthropic ×2），DeepSeek 完成率 2/4；HTTP-level 失败需要进一步定位 URL 可访问性。

## 下一步路线

1. 扩展真实站点样本库，增加更多 Greenhouse/Lever/Workday 格式的招聘 URL。
2. 对 HTTP-level 失败做分类（DNS 失败 vs 403 vs JS-render-only vs 超时），区分可修复和不可修复的 URL。
