...
> 构建 Web 自动任务 Agent，完成 AI 实习岗位搜索、JD 抽取、匹配分析和可视化报告；设计可测试的浏览器执行边界、20 任务评测集、公开招聘页 fixture 和真实浏览器 smoke 评测，输出任务成功率、有效岗位数、平均访问页面数和失败原因分布等指标。
...
- 111 个自动化测试覆盖模型、浏览器边界、抽取、验证、匹配、技能缺口汇总、简历输入、JSON 导出、评测 JSON 导出、运行历史、CLI 版本入口、环境自检、存储、报告、Dashboard 交互、workflow、LangGraph 编排、图导出、公开招聘页 fixture 和失败分类。

## 当前限制

- 真实 `browser-use` session adapter 已接入，本地可打开搜索页并生成报告；评测报告已能统计 `browser_error`、`no_pages`、`no_extracted_jobs`、`verification_filtered` 等失败类别，并已加入 Greenhouse/Lever 风格公开招聘页 fixture。
