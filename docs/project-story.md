...
- 78 个自动化测试覆盖模型、浏览器边界、抽取、验证、匹配、存储、报告、Dashboard、workflow 和评测。

## 当前限制

- 真实 `browser-use` session adapter 已接入，本地可打开搜索页并生成报告；但还没有针对招聘网站建立稳定站点评测集。
...
1. 为真实 `browser-use` 路径增加招聘网站白名单、失败原因分类和真实网页评测集，同时保留 fake browser 测试路径。
