...
第一版没有直接接入真实招聘网站，而是先用 deterministic fake browser 打通端到端闭环。这是有意设计：真实网页会受登录、验证码、反爬和页面结构变化影响，过早接入会降低项目稳定性。当前版本先保证架构、状态流、抽取、匹配、报告和评测都可复现，后续再替换真实 browser-use adapter。

## 核心能力

- Web 任务执行边界：通过 `BrowserClient` 抽象隔离 fake browser 和未来真实 `browser-use`。
...
- 可量化评测：内置 20 个任务配置，输出成功率、有效岗位数和平均访问页面数。
...
| 模块 | 职责 | 面试可讲点 |
|---|---|---|
| `browser.py` | 定义浏览器客户端协议和 fake browser | 通过接口隔离不稳定外部环境，保证测试可复现 |
...
3. BrowserClient 返回候选页面。当前版本使用 fake browser，未来可替换真实 `browser-use`。
...
> 基于 Python 构建 Web 自动任务 Agent，实现 AI 实习岗位发现、结构化 JD 抽取、岗位有效性验证、简历匹配分析、SQLite 持久化和报告生成。设计 `BrowserClient` 抽象隔离真实浏览器与 deterministic fake browser，并将工作流拆分为 Planner、Browser、Extractor、Verifier、Matcher、Reporter、Dashboard 和 Evaluation 模块。构建 20 任务评测集，统计任务成功率、有效岗位数和平均访问页面数；当前 demo 评测中 20/20 任务完成，任务成功率 1.00。

如果简历空间更短：

> 构建 Web 自动任务 Agent，完成 AI 实习岗位搜索、JD 抽取、匹配分析和可视化报告；设计可测试的浏览器执行边界与 20 任务评测集，输出任务成功率、有效岗位数和平均访问页面数等指标。
...
- 73 个自动化测试覆盖模型、浏览器边界、抽取、验证、匹配、存储、报告、Dashboard、workflow 和评测。

## 当前限制

- 真实 `browser-use` 网页操作尚未接入生产路径。
...
1. 接入真实 `browser-use` 搜索和页面读取，但保留 fake browser 测试路径。
