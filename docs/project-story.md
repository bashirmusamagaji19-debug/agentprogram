...
- 138 个自动化测试覆盖模型、浏览器边界、规则抽取、LLM demo 抽取边界、LLM demo 对比评测、验证、匹配、行动计划、简历项目改写要点、7 天执行节奏、技能缺口汇总、简历输入、seed URL、JSON 导出、评测 JSON 导出、LLM demo 评测路径、运行历史、CLI 版本入口、环境自检、fixture URL 列表、demo script、存储、报告、Dashboard 交互、Dashboard 产物链接与相对路径、输入轨迹、workflow、LangGraph 编排、图导出、公开招聘页 fixture、失败分类和 URL 级错误明细。
