...
- `dashboards/*.html`：本地 HTML Dashboard，seed URL 模式下会展示输入轨迹。
...
- 121 个自动化测试覆盖模型、浏览器边界、抽取、验证、匹配、技能缺口汇总、简历输入、seed URL、JSON 导出、评测 JSON 导出、运行历史、CLI 版本入口、环境自检、fixture URL 列表、存储、报告、Dashboard 交互与输入轨迹、workflow、LangGraph 编排、图导出、公开招聘页 fixture、失败分类和 URL 级错误明细。
