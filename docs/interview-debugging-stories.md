...
### 20. 评测脚本把"混合口径"标成了"纯 LLM"（2026-08-31 bug 排查）

- **问题**：`--evaluate-matcher` 报告写"LLM 语义匹配准确率 0.81"，但检查发现 16 条样本里 **8 条 LLM 根本没被调用**——评测把生产用的 `JobMatcher(llm_matcher=…)` 整个塞进来，规则分 ≥0.6 时不调 LLM，`llm_score` 列实为规则分。
- **排查**：对每条样本单独跑规则匹配，列出 score ≥0.6 的样本（8 条全是 match 类）——这些行的 "LLM 分数" 是规则分顶替的。也就是说 0.81 实为**混合口径**（规则优先+LLM 兜底）的准确率，报告和 work-log 里却当纯 LLM 对外说了。
- **解决**：评测改为逐条**直接调** `llm_matcher(payload)`（测的就是 LLM 组件本身），调用失败显式记 `ERR(...)` 并单独计数，不静默回退。重测纯 LLM = 0.81 (13/16)——数字巧合一致（被顶替的 8 条恰好是 LLM 能做对的），但口径从"被污染"变成"可辩护"。
- **一句话**："评测组件时要在被测组件的边界上注入和采集——把整个生产管线包进来测，测到的是管线策略不是组件能力。数字碰巧没变是运气，不是正确性的证据。"
