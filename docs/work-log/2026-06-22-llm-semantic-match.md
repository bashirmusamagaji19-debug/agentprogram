...
- ✅ DeepSeek provider 全链路 smoke 已完成（抽取 + 匹配均通过）
- 增加 LLM 语义匹配的批量评测：同一批岗位分别跑规则匹配和 LLM 匹配，人工标注匹配质量，输出对比报告

---

## 补充：DeepSeek provider 全链路 smoke

```powershell
# LLM 抽取 smoke（非结构化 JD）
.\.venv\Scripts\web-task-agent.exe --seed-url "https://example.com/jobs/unstructured-ai-agent-intern" --demo --target-count 1 --llm-extractor-provider deepseek --json-output outputs\deepseek-real-extract.json
```

结果：DeepSeek 从非结构化文本中正确提取了 Title=AI Agent Intern、Company=Example Robotics、Location=remote、Requirements=Python, LangGraph, and LLM evaluation，confidence=1.0。

```powershell
# LLM 语义匹配 smoke
.\.venv\Scripts\web-task-agent.exe --keyword "AI intern" --target-count 2 --skill Python --skill FastAPI --resume-text "Built REST APIs with FastAPI. Experienced with browser automation using Playwright." --demo --llm-match-provider deepseek --json-output outputs\deepseek-real-match.json
```

结果：
- 岗位 1 (AI Engineering Intern, requires Python/LangGraph/LLM)：规则分 0.33 → 触发 DeepSeek 语义匹配 → 得分 0.40，matched_skills=[Python]，识别出自动化经验相关性
- 岗位 2 (ML Platform Intern, requires Python/FastAPI/SQL)：规则分 0.67 ≥ 0.6 → 快速路径，不触发 LLM

验证了三层策略的正确行为：快速路径、深度路径、以及阈值控制。
