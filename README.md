...
当前可复核证据：本轮全量测试 `403 passed`；覆盖率 `90.70%`；Hybrid deterministic fixture `10/10` 循环终止、HITL `3/3` 暂停、拒绝副作用 `0`、重复副作用 `0`。这些数字来自测试输出和版本化 artifact，不代表真实招聘网站泛化准确率。
...
Online verifier 还会拒绝 `localhost`、环回、私有、链路本地和保留 IP，并在每次详情页请求前检查 DNS 解析结果，防止开放搜索结果诱导服务端访问内网资源；这是演示服务的 SSRF 基础防护，不替代生产环境的网络出口策略。
